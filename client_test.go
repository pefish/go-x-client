...
type gotwiClientField struct {
	AuthenticationMethod go_x_client.AuthenticationMethod
	AccessToken          string
	OAuthToken           string
...
func (f gotwiClientField) build() *go_x_client.Client {
	c := &go_x_client.Client{
		Client: f.Client,
	}
...
func Test_NewClient(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(tt *testing.T) {
			tt.Setenv("X_API_KEY", c.envAPIKey)
			tt.Setenv("X_API_KEY_SECRET", c.envAPIKeySecret)
			...
			gc, err := go_x_client.NewClient(c.in)
			if c.wantErr {
				assert.Error(tt, err)
				assert.Nil(tt, gc)
				return
...
func Test_NewClientWithAccessToken(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(tt *testing.T) {
			gc, err := go_x_client.NewClientWithAccessToken(c.in)
			if c.wantErr {
				assert.Error(tt, err)
				assert.Nil(tt, gc)
				return
...
func Test_newRequest(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(tt *testing.T) {
			r, err := go_x_client.ExportNewRequest(context.Background(), c.endpoint, c.method, c.p)
			if c.wantErr {
				assert.Error(tt, err)
				assert.Nil(tt, r)
				return
...
func Test_CallAPI(t *testing.T) {
	cases := []struct {
		name            string
		mockInput       *mockInput
		clientInput     *go_x_client.NewClientInput
		endpoint        string
		method          string
		envAPIKey       string
		envAPIKeySecret string
	...
	for _, c := range cases {
		t.Run(c.name, func(tt *testing.T) {
			...
			client, _ := go_x_client.NewClient(in)
...
func Test_resolveNon2XXResponse(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(tt *testing.T) {
			e, err := go_x_client.ExportResolveNon2XXResponse(c.res)
			if c.wantErr {
				assert.Error(tt, err)
				assert.Nil(tt, e)
				return
